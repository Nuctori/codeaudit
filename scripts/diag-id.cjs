...
const withDirect = chain0.filter(
	(v) => v.chunk.direct && v.chunk.direct.size > 0,
);
...
const byEff = {};
for (const v of chain0)
	for (const e of v.effects) byEff[e] = (byEff[e] ?? 0) + 1;
