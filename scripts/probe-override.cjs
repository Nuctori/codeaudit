...
const { scanProject, loadEffectOverrides, validateEffectOverride, applyEffectOverrides, defaultPacks } = require("../dist/index.js");
...
async function main() {
  ...
  if (!root) { console.error("用法: node scripts/probe-override.cjs <项目根>"); process.exit(1); }
  const report = await scanProject(root, { useCache: false, effectOverrides: overrides });
  const s = report.stats;
  console.log(`chunks ${s.chunks} | pure ${s.pure} | impure ${s.impure} | unknown ${s.unknown}`);
