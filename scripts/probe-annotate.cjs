...
async function main() {
  ...
  const report = await scanProject(root, { useCache: false, annotations: new Map(ann.map((a) => [a.id, a.verdict])) });
  const s = report.stats;
  console.log(`回读后: pure ${s.pure} | impure ${s.impure} | unknown ${s.unknown}`);
  ...
  console.log(`unknown 变化: ${dropped > 0 ? `-${dropped} (标注生效)` : "无变化 (标注全被拒)"}`);
}
