...
if (!reportPath || !annPath) {
	console.error(
		"用法: guard-annotations.cjs <scan-report.json> <annotations.json>",
	);
	process.exit(2);
}
...
const PURE_RISK = [
	...
	/Instantiate\(/i,
	/transform\./i,
	/DOTween|DOFade|DOAnchor/i,
	/\.position\s*=/i,
	/\.rotation\s*=/i,
	/Debug\.(Log|LogError|LogWarning)/i,
	/await\s+\w+.*(Async|Rpc|RPC)/i,
	/DataCollect|\.Track\(/i,
	/Resources\.Load/i,
	/AssetBundle|LoadCachedSprites|LoadAsset/i,
	/PlayerPrefs/i,
	/File\./i,
	/EditorPrefs|ApplyModifiedProperties/i,
	/\.localScale\s*=/i,
	/\.isKinematic/i,
];
...
for (const a of ann) {
	...
	try {
		body = fs
			.readFileSync(path.join(root, v.chunk.file), "utf8")
			.split("\n")
			.slice(v.chunk.startLine - 1, v.chunk.endLine)
			.join("\n");
	} catch {
		continue;
	}
	const risks = [];
	if (a.verdict === "PURE") {
		for (const re of PURE_RISK)
			if (re.test(body)) {
				risks.push(re.source);
				break;
			}
	} else if (a.verdict === "IMPURE") {
		if (IMPURE_RISK.every((re) => re.test(body)))
			risks.push("方法体无效应调用迹象（纯字符串/遍历？）");
	}
	if (risks.length > 0) {
		riskCount++;
		console.error(
			`⚠ ${a.verdict} ${v.chunk.name} @ ${v.chunk.file}:${v.chunk.startLine} — ${risks[0]}`,
		);
	}
}
console.error(
	`护栏检查：${ann.length} 条标注，${riskCount} 条风险（标注者复核——不自动改）`,
);
