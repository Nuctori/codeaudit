...
for (const v of srcs) {
	console.log(
		`  ${String(v.chunk.calls.size).padStart(4)}  ${v.chunk.name}  [${[...v.effects].join(",")}]  ${v.chunk.file.split("/").slice(-2).join("/")}:${v.chunk.line}`,
	);
}

console.log("\n=== 3. 假纯候选（PURE 但含可疑形态：直接调 io 库方法名） ===");
const ioMethods = new Set([
	"Console",
	"WriteLine",
	"Debug",
	"Log",
...
for (const v of by(PURE)) {
	for (const c of v.chunk.calls) {
		...
		if ([...ioMethods].some((m) => last.includes(m))) {
			console.log(
				`  [FP?] ${v.chunk.name} calls ${last} @ ${v.chunk.file.split("/").slice(-2).join("/")}:${v.chunk.line}`,
			);
			fp++;
			break;
		}
