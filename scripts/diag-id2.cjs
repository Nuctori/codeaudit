...
if (c0) {
  console.log("chain=0 样本:", c0.chunk.name, "direct", JSON.stringify(c0.chunk.direct), "effects", [...c0.effects], "calls", JSON.stringify([...c0.chunk.calls].slice(0, 6)), "unknownSites", c0.chunk.unknownSites);
}
...
for (const v of unk) {
  ...
  const k = v.chunk.parseError ? "parseError" : (calls.includes("?") ? "has?" : (calls.length === 0 ? "no-calls" : "has-calls"));
