 ...
 *
 * 方向安全（迭代9 精确声明）：精确/子树匹配零假阳性；盲区（下标写 d[k]=、项目外写者、
 * 读者读整对象、裸下标值读）只漏报不假报；⊤ 降级与同名异对象
 * （跨模块/作用域同名）可过近似——仅影响耦合元数据可见性，不进判定。
 *
 * 残余（文档化，README 已知限制）：项目外写者（测试夹具/框架注入）不可见 → 漏报；
 * 调用结果写 f().x= 在迭代26 起由 extractor 的 subscriptRoot 兜底降级为 "f.⊤"（镜像读侧），
 * 局部 subscript 根（dd[k].x= 的 dd 局部）仍过近似为 "dd.⊤"——有界、仅耦合元数据。
 */
