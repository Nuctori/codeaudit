...
exports.annotationBudget = annotationBudget;
exports.influenceAnalysis = influenceAnalysis;
exports.compareReports = compareReports;
exports.unknownKeysOf = unknownKeysOf;
exports.annotationCompare = annotationCompare;
exports.changedImpact = changedImpact;
exports.annotationCurve = annotationCurve;
const types_1 = require("./types");
const tarjan_1 = require("./tarjan");
function annotationBudget(chunks) {
    const byKey = new Map(chunks.map((c) => [c.key, c]));
    const edges = new Map();
    for (const c of chunks) {
        const s = new Set();
        for (const t of c.calls)
            if (t !== types_1.UNKNOWN_TARGET && byKey.has(t))
                s.add(t);
        edges.set(c.key, s);
    }
    // 逆拓扑：callee 分量下标更小（tarjan 契约）
    const sccs = (0, tarjan_1.tarjan)(byKey.keys(), edges);
    const comp = new Map();
    ...
    sccs.forEach((s, k) => {
        for (const i of s)
            for (const t of edges.get(i))
                if (comp.get(t) !== k)
                    succ[k].add(comp.get(t));
    });
    const sourceOf = new Map(); // 分量 → 含 `?` 的 chunk key 列表
    for (const c of chunks) {
        if (c.calls.has(types_1.UNKNOWN_TARGET)) {
    ...
    const Down = sccs.map(() => new Set());
    const inflByComp = new Map();
    const reachByComp = new Map(); // 源分量 → 受影响的 chunk keys
    for (let k = 0; k < sccs.length; k++) {
        const set = new Set();
        if (sourceOf.has(k))
            set.add(k);
        for (const s2 of succ[k])
            for (const s of Down[s2])
                set.add(s);
        Down[k] = set;
        for (const s of set) {
            inflByComp.set(s, (inflByComp.get(s) ?? 0) + sccs[k].length);
            const arr = reachByComp.get(s) ?? [];
            for (const key of sccs[k])
                arr.push(key);
    ...
    for (const [k, keys] of sourceOf)
        srcCount.set(k, keys.length);
    const deps = new Map();
    for (const c of chunks) {
        let n = 0;
        for (const s of Down[comp.get(c.key)])
            n += srcCount.get(s) ?? 0;
        if (n > 0)
            deps.set(c.key, n);
    }
...
function influenceAnalysis(chunks) {
...
function compareReports(a, b) {
    ...
    const out = [];
    const setsEqual = (x, y) => x.size === y.size && [...x].every((e) => y.has(e));
    for (const va of a) {
        ...
        if (va.purity === vb.purity && va.chain === vb.chain && setsEqual(va.effects, vb.effects))
            continue;
        out.push({
            key: va.chunk.key,
            file: va.chunk.file,
 ...
 */
function unknownKeysOf(verdicts) {
    const out = new Set();
    for (const v of verdicts)
        if (v.purity === types_1.Purity.UNKNOWN)
            out.add(v.chunk.key);
    return out;
}
/**
 ...
 */
function annotationCompare(budget, unknownKeys) {
    return (a, b) => {
...
function changedImpact(verdicts, changedFiles) {
    ...
    const callers = new Map(); // callee key → caller keys
    for (const v of verdicts) {
        for (const t of v.chunk.calls) {
            ...
            if (arr)
                arr.push(v.chunk.key);
            else
                callers.set(t, [v.chunk.key]);
        }
    }
    ...
    const queue = seeds.map((k) => [k, 0, null, null]);
    const out = [];
    for (let i = 0; i < queue.length; i++) {
        const [k, d, via, viaName] = queue[i];
        const c = byKey.get(k);
        if (c)
            out.push({ key: k, file: c.file, name: c.name, line: c.line, depth: d, via, viaName });
    ...
    for (const u of order) {
        if (!annotated.has(u)) {
            ...
            for (const w of budget.released.get(u) ?? []) {
                if (!counted.has(w))
                    continue; // 目标之外的 chunk（IMPURE 带未知依赖）不计数
                const n = (need.get(w) ?? 0) - 1;
                need.set(w, n);
                if (n === 0)
                    remaining -= weight?.get(w) ?? 1;
            }
        }
