...
export interface ScanOptions {
	...
	readonly effectOverrides?: Readonly<
		Record<string, Partial<import("../lang/effectOverride").EffectTables>>
	>;
}
...
function computeFingerprint(): string {
	...
	try {
		...
		for (const f of readdirSync(base))
			if (f.endsWith(".js") || f.endsWith(".ts")) files.push(f);
		for (const f of readdirSync(join(base, "packs")))
			if (f.endsWith(".js") || f.endsWith(".ts")) files.push("packs/" + f);
		files.sort();
...
function validFacts(f: RawFileFacts | undefined): f is RawFileFacts {
	if (
		...
		return false;
	if (
		f.moduleBindings !== undefined &&
		(typeof f.moduleBindings !== "object" || f.moduleBindings === null)
	)
		return false;
	if (f.chunks.length > MAX_CACHE_CHUNKS) return false; // 超预算整包拒绝 → 全量重扫（防 316MB 缓存放大 DoS）
	// 迭代38 规则8：classExtends/hasDynamicExtends 可选字段形状校验（畸形缓存 → 全量重扫）
	if (
		f.classExtends !== undefined &&
	...
	for (const c of f.chunks) {
		if (
			...
			typeof c.normText !== "string" ||
			typeof c.line !== "number" ||
			!Array.isArray(c.calls) ||
			!Array.isArray(c.assigned)
		)
			return false;
		...
		if (!Array.isArray(c.stateWrites) || !Array.isArray(c.stateReads))
			return false;
		totalCalls += c.calls.length;
		if (totalCalls > MAX_CACHE_CALLS) return false;
	}
	for (const i of f.imports) {
		if (!i || typeof i.local !== "string" || typeof i.module !== "string")
			return false;
	}
	for (const c of f.chunks) {
		for (const call of c.calls) {
			if (
				!call ||
				typeof call.attr !== "string" ||
				(call.obj !== null && typeof call.obj !== "string") ||
				(call.receiver !== null && typeof call.receiver !== "string")
			)
				return false;
		}
	}
...
export async function scan(opts: ScanOptions): Promise<ScanReport> {
	...
	if (opts.useCache && cachePath) {
		try {
			...
			if (statSync(cachePath).size > MAX_CACHE_FILE_BYTES)
	...
	for (const [file, pack] of sortedFiles) {
		...
		if (
			...
			hit.facts.file === file && // 键即身份：缓存条目不得伪造 facts.file 指向兄弟文件
	...
	if (ann && ann.size > 0) {
		for (const [k, v] of ann)
			if (v === "PURE") pureAnnIds.add(k.split("\u0000")[1] ?? k);
	}
