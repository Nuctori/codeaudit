	...
	const visit = (n: SyntaxNode): void => {
		if (n.type === "import_statement") {
			...
			if (src) {
				...
				if (nsexp) {
					const nsNode = nsexp.children.find(
						...
						if (nameNode.type === "identifier") {
							...
							for (const prop of nameNode.children) {
								if (prop.type === "shorthand_property_identifier_pattern") {
									...
									const val =
										prop.childForFieldName("value") ?? prop.children[1];
									if (
										key &&
										val &&
										(val.type === "identifier" ||
											val.type === "property_identifier")
									) {
...
export function resolveEsmModule(
	...
	for (const ext of extensions)
		candidates.push(posix(normalize(join(base, "index" + ext))));
...
const impureModules: Record<string, Effect | readonly string[]> = {
	...
	crypto: [
		...
		"randomInt:random",
		"generateKey:random",
		"generateKeySync:random",
		"generateKeyPair:random",
		"generateKeyPairSync:random",
...
export const typescriptPack: LangPack = {
	...
	// 迭代39 P2-1：AST 形状投影（π 数据侧——extractor 节点类型判定走此表；javascript 经 spread 继承）
	astShapes: {
		writeStmts: [],
		writeAssigns: ["assignment_expression", "augmented_assignment_expression"],
		writeUpdates: ["update_expression"],
		writeUnary: [],
		memberNodes: ["member_expression"],
		memberWrapNodes: [],
		...
		stmtWrapNodes: ["export_statement", "expression_statement"],
		...
		exportStmtNodes: ["export_statement"],
	},
	...
	resolveModule: (module, fromFile, projectFiles) =>
		resolveEsmModule(module, fromFile, projectFiles, [
			".ts",
			".tsx",
			".js",
			".jsx",
