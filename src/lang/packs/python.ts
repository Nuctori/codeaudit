...
const impureModules: Record<string, Effect | readonly string[]> = {
	...
	boto3: "db",
	paramiko: "io",
	pickle: ["load:fs", "dump:fs"],
	json: ["load:fs", "dump:fs", "dumps:p", "loads:p"],
	csv: "fs",
	logging: "io",
	...
	pathlib: [
		...
		"Path.absolute:p",
		"Path.cwd:fs",
		"Path.home:fs", // 读进程工作目录/HOME 环境（F20：非纯——S1 通道）
		"Path.read_text:fs",
		"Path.read_bytes:fs",
		"Path.write_text:fs",
		"Path.write_bytes:fs",
	...
	datetime: [
		...
		"utcnow:clock",
		"fromtimestamp:clock",
		"utcfromtimestamp:clock",
		// 点连形态（import datetime; datetime.datetime.now()）：全名命中前末段回退，此处点连变体补齐
		"datetime.now:clock",
		"datetime.today:clock",
		"datetime.utcnow:clock",
		"datetime.utcfromtimestamp:clock",
		"datetime.fromtimestamp:clock",
		"datetime.combine:p",
		"datetime.strptime:p",
		"datetime.strftime:p",
		"date.today:clock",
		"date.fromtimestamp:clock",
		"date.fromordinal:p",
		"time.utcfromtimestamp:clock",
		// ":p" = 纯转换成员（不触时钟/io）
		"combine:p",
		"fromisoformat:p",
		"strftime:p",
...
export const pythonPack: LangPack = {
	...
	astShapes: {
		writeStmts: ["global_statement", "nonlocal_statement"],
		writeAssigns: ["assignment", "augmented_assignment"],
		writeUpdates: [],
		writeUnary: [],
		...
		callShapes: ["call"],
		ctorCallNodes: [],
		paramNodes: ["parameter", "typed_parameter"],
		throwNodes: ["raise_statement"],
			...
			if (n.type === "import_statement") {
				...
				for (const c of n.children) {
					if (c.type === "dotted_name") {
						...
						const alias =
							c.childForFieldName("alias") ?? c.children[c.children.length - 1];
						if (mod && alias) {
	...
	resolveModule(
		...
		const suffixes = [
			posix(normalize(modPath + ".py")),
			posix(normalize(join(modPath, "__init__.py"))),
		];
