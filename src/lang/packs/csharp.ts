...
const pureGlobals = new Set<string>([
  ...
  "Vector2", "Vector3", "Vector4", "Quaternion", "Color", "Rect", // Unity 数学结构
  "Mathf", // Unity 数学
  "StringBuilder", // 可变字符串——纯（无 io）
  // 内建值类型静态方法（int.Parse/TryParse 等——纯计算；迭代19 C#）
  "int", "long", "float", "double", "decimal", "bool", "char", "byte",
  "sbyte", "short", "ushort", "uint", "ulong", "object", "Array", "List", "Dictionary",
]);
...
const builtinTypeEffects: Record<string, Record<string, "pure" | "hof">> = {
  string: {
    ToUpper: "pure", ToLower: "pure", Trim: "pure", Substring: "pure", Replace: "pure",
    Split: "pure", Contains: "pure", StartsWith: "pure", EndsWith: "pure", IndexOf: "pure",
    LastIndexOf: "pure", Concat: "pure", Format: "pure", Join: "pure", Compare: "pure",
    Equals: "pure", GetHashCode: "pure", ToCharArray: "pure", PadLeft: "pure", PadRight: "pure",
    Remove: "pure", Insert: "pure", ToString: "pure", IsNullOrEmpty: "pure", IsNullOrWhiteSpace: "pure",
  },
  number: {
    ToString: "pure", CompareTo: "pure", Equals: "pure",
  },
...
const builtinMethodReturns: Record<string, Record<string, string>> = {
  string: { ToUpper: "string", ToLower: "string", Trim: "string", Substring: "string" },
};
...
const classNodes = ["class_declaration", "struct_declaration", "interface_declaration"];
...
const nestingNodes = [
  ...
  "switch_statement", "try_statement", "method_declaration", "local_function_statement",
  "lambda_expression", "anonymous_method_expression", "class_declaration",
];
const selfNames = ["this", "base"];
const assignmentTargets = [
  "assignment_expression", "local_declaration_statement", "variable_declaration",
];
...
const pureModules = new Set<string>([
  ...
  "System.Text", "System.Text.RegularExpressions", "System.Globalization",
  "System.Threading", "System.Threading.Tasks", "System.Numerics",
]);
...
export function extractCSharpImports(root: SyntaxNode): RawImport[] {
  ...
  const visit = (n: SyntaxNode): void => {
    if (n.type === "using_directive" || n.type === "using_declaration") {
      const name = n.childForFieldName("name") ?? n.children.find((c) => c.type === "identifier" || c.type === "qualified_name");
