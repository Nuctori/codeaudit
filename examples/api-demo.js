...
async function main() {
	...
	console.log(
		`[scan] ${report.stats.chunks} chunks / ${report.stats.files} files`,
	);
	console.log(
		`  pure ${pure} / impure ${impure} / unknown ${unknown} (unknown-rate ${(unknownRate * 100).toFixed(1)}%)`,
	);
	...
	console.log(
		`[analyzeChange] ${changed.join(", ")} → ${impact.summary.affectedChunks} 个受影响调用者`,
	);
	...
	console.log(
		`  证据质量: 未知率 ${(risk.evidence.unknownRate * 100).toFixed(1)}% / 未解析站点 ${(risk.evidence.missingSiteRate * 100).toFixed(1)}%`,
	);

	// 4. 证明完整度（Θ：标注多少未知到完整）
	const proof = proofCompleteness(report.verdicts, {
		weighted: true,
		targetTheta: 0.9,
	});
	console.log(
		`[proofCompleteness] Θ=${proof.theta.toFixed(3)} 标到 0.9 需 ${proof.budgetToTarget ?? "不可达"} 条`,
	);

	// 5. 拓扑健康度
	const topo = graphMetrics(report.verdicts);
	console.log(
		`[graphMetrics] 密度 ${topo.density.toFixed(3)} / 环 ${topo.cyclicComponents} / 深度 ${topo.dagDepth} / 自环 ${topo.selfLoopCount}`,
	);
}
