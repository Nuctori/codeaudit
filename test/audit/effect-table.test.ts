...
let dir: string;
beforeAll(() => {
	dir = mkdtempSync(join(tmpdir(), "codeaudit-c5-"));
});
afterAll(() => {
...
const CASES: Array<[string, string, Set<string>]> = [
	[
		"net",
		'public void net() { UnityWebRequest.Get("http://x"); }',
		new Set(["net"]),
	],
	[
		"scene",
		'public void scene() { SceneManager.LoadScene("x"); }',
		new Set(["state"]),
	],
	[
		"anim",
		'public void anim() { this.animator.SetTrigger("x"); }',
		new Set(["io"]),
	],
	[
		"prefs",
		'public void prefs() { PlayerPrefs.SetFloat("b", 1f); }',
		new Set(["state"]),
	],
	...
	[
		"env",
		'public void env() { Environment.GetEnvironmentVariable("X"); }',
		new Set(["io"]),
	],
	["proc", 'public void proc() { Process.Start("x"); }', new Set(["io"])],
	[
		"console",
		'public void console() { Console.WriteLine("x"); }',
		new Set(["io"]),
	],
	[
		"destroy",
