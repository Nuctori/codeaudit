...
let dir: string;
beforeAll(() => {
	dir = mkdtempSync(join(tmpdir(), "codeaudit-src-"));
});
afterAll(() => {
...
describe("--sources 效应源（迭代17 视角 1）", () => {
	...
	it("json 模式 --sources 顶层 sources 数组（R2-1）", () => {
		const root = project("srcjson", {
			"a.js":
				"module.exports.save = function save() { return console.log('s'); };\n",
		});
		const cli = join(
		...
		const out = execFileSync(
			"node",
			[cli, "scan", root, "--no-cache", "--sources", "--json"],
			{ encoding: "utf8" },
		);
