...
let dir: string;
beforeAll(() => {
	dir = mkdtempSync(join(tmpdir(), "codeaudit-contract-"));
});
afterAll(() => {
...
describe("维度31: JSON 输出 schema", () => {
	it("--changed json 模式 stdout 纯净（20b07ca 混合流修复回归）", () => {
		const root = project("riskjson", {
			"a.ts": "export function f() { console.log('x'); }\n",
		});
		const res = spawnSync(
			"node",
			[CLI, "scan", ".", "--changed", "a.ts", "--format", "json", "--no-cache"],
	...
	it("文本输出包含三个分组标题与 STATS 行", async () => {
		...
		const out = execFileSync("node", [CLI, "scan", root, "--no-cache"], {
			encoding: "utf8",
		});
...
describe("维度32: unknowns 导出（AI 标注闭环）", () => {
	it("导出文件存在、含 prompt、只含 UNKNOWN", async () => {
		...
		execFileSync(
			"node",
			[CLI, "scan", root, "--no-cache", "--unknowns", outFile],
			{ encoding: "utf8" },
		);
	...
	it("--annotations 回读减少 UNKNOWN（CLI 端到端）", async () => {
		const root = project("anncli", {
			"a.py": "import weirdlib\ndef f():\n    weirdlib.run()\n",
		});
		const before = JSON.parse(
			execFileSync("node", [CLI, "scan", root, "--no-cache", "--json"], {
				encoding: "utf8",
			}),
		);
		...
		writeFileSync(
			annFile,
			JSON.stringify([{ id: f0.chunk.id, verdict: "PURE" }]),
		);
		const after = JSON.parse(
			execFileSync(
				"node",
				[CLI, "scan", root, "--no-cache", "--annotations", annFile, "--json"],
				{ encoding: "utf8" },
			),
		);
		expect(after.verdicts.find((v: any) => v.chunk.name === "f")!.purity).toBe(
			0,
		);
	});
	...
	it("标注闭环：标注 → 语料累积 → 幂等（CLI 端到端）", async () => {
		...
		execFileSync(
			"node",
			[CLI, "scan", root, "--no-cache", "--unknowns", u1, "--corpus", corpus],
			{ encoding: "utf8" },
		);
	...
	it("unknowns 导出含 code 字段（工作台——chunk 行区间源码，迭代44-r3）", async () => {
		...
		execFileSync("node", [CLI, "scan", root, "--no-cache", "--unknowns", u], {
			encoding: "utf8",
		});
