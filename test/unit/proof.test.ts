...
function v(key: string, opts: { purity?: number; calls?: string[]; file?: string } = {}): Verdict {
  const purity = opts.purity ?? Purity.PURE;
  return {
    chunk: {
      id: key, key, name: key, file: opts.file ?? "f.ts", line: 1, endLine: 2, nesting: 0,
      direct: new Set(), calls: new Set(opts.calls ?? []), unknownSites: 0, unknownCalls: [],
      thrownTypes: [], catches: [], stateWrites: [], stateReads: [],
    },
    purity,
    effects: new Set(purity === Purity.IMPURE ? ["io"] : []),
    chain: purity === Purity.PURE ? Infinity : 0,
    chainDev: Infinity, chainCertain: true,
    inDegree: 0, outDegree: 0, chainPath: [], throwsTypes: [], stateDeps: [],
  };
}
...
describe("证明完整度 proofCompleteness", () => {
  ...
  it("预算序确定性 + 曲线单调不增 + 乱序输入一致", () => {
    ...
    expect(p1.curve.every((x, i) => i === 0 || x <= p1.curve[i - 1]!)).toBe(true);
