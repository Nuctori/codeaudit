...
function v(key: string, opts: { calls?: string[]; chain?: number; chainCertain?: boolean; purity?: number; unknownSites?: number } = {}): Verdict {
  return {
    chunk: {
      key,
      file: `${key}.js`,
...
describe("graphMetrics（迭代14 视角 3 实施）", () => {
  ...
  it("与 stats.cycles 一致性对拍（真实 scan）", async () => {
    ...
    writeFileSync(join(dir, "a.js"), "module.exports = function a() { return b(); };\nmodule.exports.b = function b() { return a(); };\n");
  ...
  it("随机 DAG 对拍：dagDepth === 朴素 DFS 最长路径", () => {
    ...
    for (let t = 0; t < 100; t++) {
      ...
      const verdicts = [...calls.entries()].map(([key, c]) => v(key, { calls: c }));
