...
**仍未做（用户可选）**：加权链（阅读量语义）、谱诊断（λ₂）、正确性清单（缓存信任边界/深度上限/--strict 崩溃）。

## 步骤 4：数学层补全四件（三方评审 → 全部否决，2026-08-10）

按用户「每一步三方评审」流程，对 state 原子 / 加权链 / 谱诊断 λ₂ / 注解账本 召开数学家 + AI 工程师 + Jeff Dean 评审会。裁决：

- **state 效应原子**：历史否决（Jeff Dean）——「effects={state} 却不 IMPURE」心智冲突。2026-08-11 用户需求覆盖：state 已实现且驱动判定（任何效应 → IMPURE，含 state）——否决理由（不驱动判定）不再适用。
- **加权链**：否决——chain 是四处耦合的判定量（计算/排序/区间/机检不变量），换权重=全语义重写；1+log₂|SCC| 是拍脑袋阅读量模型；真实替代（影响面/deps）已落地。
- **谱诊断 λ₂**：否决——代数连通度是无向图概念，调用图对称化后多分量使 λ₂ 恒 0；TS 运行时无 numpy，特征分解成本爆表；数字无动作语义。
- **注解账本**：否决（完整版 = 被 §10.2 数据否决的类型层还魂；廉价版 TS-only 计数无动作绑定）——§10.2 (b) 实测注解参数触发率 0%。
- 评审同时确认：AI 工程师指出四件均可实现且有修正方案（state 需 analyze 过滤、加权链仅 1 测试回归、λ₂ 需规范化 Laplacian+最大分量、账本需标注"假设性上限"）——可行性不构成做它的理由，价值/成本是。
- **结论**：四件不实施，符合项目停止准则（只修正确性 bug、不立项新特性）。若未来出现 TS 重度用户诉求（注解反事实指导），可重审注解账本廉价版。
