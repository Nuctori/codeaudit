...
| UNKNOWN rate | 28.1% | 25.0% | 判定分布变化（见下） |
| IMPURE / PURE | 9449/8590 | 10537/8092 | C# 字段写可见后 state 效应增加（写侧对偶⑦） |

- **判定分布变化说明**：UNKNOWN 28.1%→25.0%、IMPURE 9449→10537——`transform.position = x` 等 C# 字段写此前不可见（假 UNKNOWN/假 PURE 方向），写侧对偶⑦后正确判 state → IMPURE。这是**正确化**（C# 字段写从不可见到可见），非误伤（T3 用例证语义）。
