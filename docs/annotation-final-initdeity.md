...
unknown-rate：24.4 → 23.9（0.5pp）→ 21.8（**2.1pp**——批组轮大释放）→ 21.0（0.8pp）——**仅两轮 <1pp，未连续触发**（审计修正：此前声称"连续三轮 <1pp"与数据不符）。但工具侧可修项已清零（attribute/using/global/局部读/枚举/编译期操作符/foreach-catch），后续降幅将依赖标注——**建议引擎迭代转收口，转入项目侧与标注运营**（工具侧边际收益已见底）。
