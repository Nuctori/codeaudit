...
5. **排序不混合量纲**：每个输出视图用**单一量纲**排序（治理清单=直接调用者数、复杂度=圈复杂度、效应源=调用点、未知=影响面……），不存在拍脑袋的加权求和；量纲间平手才用确定性 tiebreak（chain → key 字典序）。（输出契约——不约束计算语义，仅约束渲染。）
...
npm test             # 446 个测试：单元 + 多语言 E2E + 合成大库 + 自扫描 + 交叉审计 + 继承/多态反例 + 形状契约 + 数学层回归（开发需 Node ≥20，vitest 4）

# 扫描
node dist/cli.js scan ./src
node dist/cli.js scan ./src --format json
node dist/cli.js scan ./src --top 20            # 治理清单前 N（逆向依赖优先 → 直接调用者数降序——反向路径先修，被最多人引用的非纯次之）
node dist/cli.js scan ./src --topology          # 拓扑健康度（密度/环/深度/自环/多入口纠缠环/桥/割点/源汇/逆向依赖边 + 人类解读）
...
- **C# 属性访问器（迭代40 B5 闭合）**：属性声明提独立 chunk（自定义 getter/setter 体调用归属属性，自动属性=空 chunk）；`obj.Prop` 读取形态建调用边——自定义 getter 的 io 传染读取方（此前假纯）；字段/自动属性/不存在成员读取判纯（C# 静态语义：不执行用户代码）。残余：`obj?.Prop` 条件访问读取、TS/JS/Python 属性读取（M6 已修 TS/JS，Python 保持文档化接受）、项目外子类/写者——见 docs/technical-debt.md M_out 清单（假纯可能通道，逐条声明触发条件）。
- **C# 事件订阅（迭代43 B 闭合）**：事件间接层 + 订阅/触发双通道——`evt += h` 订阅跨方法关联，`evt(...)`/`evt.Invoke()`/`evt?.Invoke()` 触发展开 handler 闭包（S2 过近似）；private 事件触发端确定判定、非 private 附加 `?`（外部订阅不可见）；`+=` 双语义（state 写 ⊕ 订阅边）。残余：跨实例触发（`x.evt(...)`）接收者不可证 → `?`、UnityEvent 模型外派发、事件本身不可标注（无 chunk）。
...
446 个测试，五层验证（32 维交叉审计见 [AUDIT.md](AUDIT.md)，另有数学层回归组）：
